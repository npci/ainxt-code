...
export class SessionTreeProvider implements vscode.TreeDataProvider<AgentNode | ChildNode> {
  ...
  private getAgentNodes(): AgentTreeItem[] {
    // ainxt fork: only ever surface the single governed "ainxt" agent, even if
    // a user's acp.agents setting defines others.
    return getAgentNames().filter(name => name === 'AiNxt').map(name => {
