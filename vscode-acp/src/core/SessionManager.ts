...
import { RequestError, ClientSideConnection } from '@agentclientprotocol/sdk';
...
import { isSafeAuthUrl } from '../utils/GatewaySecurity';
...
export class SessionManager extends EventEmitter {
  ...
  private getWorkspaceCwd(): string {
    // Honour the user-configured default working directory first (acp.defaultWorkingDirectory).
    // Falls back to the first workspace folder, then process.cwd().
    const configured = vscode.workspace.getConfiguration('acp').get<string>('defaultWorkingDirectory', '').trim();
   ...
   */
  /** Locally-persisted conversation history for an agent (recent-first). */
  listLocalSessions(agentName: string): Array<{ sessionId: string; title?: string; firstPrompt?: string; lastActiveAt?: string }> {
    return (this.historyStore?.list(agentName, this.getWorkspaceCwd()) ?? []).map((e) => ({
      sessionId: e.sessionId, title: e.title, firstPrompt: e.firstPrompt, lastActiveAt: e.lastActiveAt,
    }));
  }
  ...
  /**
   * Kill and respawn an agent so spawn-time environment (gateway URL,
   * insecure-http gate) is re-read. Used when connection settings change.
   */
  async reconnectAgent(agentName: string): Promise<SessionInfo> {
    await this.disconnectAgent(agentName);
    return this.connectToAgent(agentName);
  }

  /**
   * Connect and RESUME the most recent conversation if possible, instead of
   * always starting a blank one. In leader mode the agent's session actors
   * live in the persistent leader daemon, so after an extension reload/restart
   * we can `session/load` the last session and get its full history back —
   * this is what keeps multi-turn context alive across reloads. Falls back to
   * a fresh session if there's nothing to resume or the load fails.
   */
  async connectOrResume(agentName: string): Promise<SessionInfo> {
    // Already have a live session with this agent — reuse it.
    const existingSessionId = this.agentSessions.get(agentName);
    if (existingSessionId && this.sessions.has(existingSessionId)) {
      this.activeSessionId = existingSessionId;
      this.emit('active-session-changed', existingSessionId);
    ...
    // Ensure the process is connected (no session yet) so we know its caps.
    const connInfo = await this.ensureConnected(agentName);
    const cwd = this.getWorkspaceCwd();
    const caps = this.capabilities.get(agentName);

    // Try to resume the most recent persisted session for this agent+cwd.
    if (caps?.load) {
      const recent = this.historyStore?.list(agentName, cwd) ?? [];
      if (recent.length > 0) {
        try {
          return await this.loadSession(agentName, recent[0].sessionId);
        } catch (e) {
          // loadSession prunes the entry if the agent says it's gone; either
          // way we fall through to a fresh session below.
          log(`connectOrResume: resume failed, starting fresh — ${(e as Error)?.message ?? e}`);
        }
      }
    }

    // Fresh session on the already-connected process (no re-spawn).
    const agentId = this.findAgentIdForConnection(connInfo);
    if (!agentId) {
      // Connection went away; fall back to the full spawn+session path.
      return this.connectToAgent(agentName);
    }
  ...
  private async runAuthFlow(
    ...
    try {
      ...
      // AiNxt in-IDE SSO: start the agent-side auth flow, then open the browser
      // URL it publishes via `ainxt.dev/auth/get_url` (loopback callback, or a
      // device code we surface). Without this the OIDC flow never opens a browser
      // and sign-in can't complete. Non-OIDC methods (api_key/cached_token) don't
      // publish a URL, so the get_url call resolves empty / rejects — harmless.
      await this.driveAuthBrowser(connInfo.connection, selectedMethod.id);
  ...
  /**
   * Drive the agent-side auth flow for `methodId`: start `authenticate`, then
   * open the browser URL it publishes via `auth/get_url` (loopback) or surface
   * the device code. Non-URL methods (api_key/cached) simply resolve.
   */
  private async driveAuthBrowser(conn: ClientSideConnection, methodId: string): Promise<void> {
    const authPromise = conn.authenticate({ methodId, meta: { headless: false } } as never);
    conn.extMethod('ainxt.dev/auth/get_url', {})
      .then((res: Record<string, unknown>) => {
        const url = res?.auth_url as string | undefined;
        const mode = res?.mode as string | undefined;
        const code = res?.user_code as string | undefined;
        if (!url) { return; }
        // The agent supplies this URL over the ACP channel and we hand it to the
        // user's browser, so validate it before opening: a compromised agent must
        // not be able to launch an arbitrary or non-TLS destination, and sending
        // sign-in credentials over cleartext would expose them (CWE-1204/CWE-319).
        if (!isSafeAuthUrl(url)) {
          logError(`Refusing to open sign-in URL "${url}": must be https:// or a loopback address.`);
          void vscode.window.showErrorMessage(
            'AiNxt sign-in was cancelled: the agent supplied an unsafe sign-in URL. ' +
              'Expected an https:// address or a local callback.',
          );
          return;
        }
        if (mode === 'device' && code) {
          void vscode.env.clipboard.writeText(code);
          void vscode.window.showInformationMessage(`AiNxt sign-in: enter code ${code} (copied) in the browser.`);
        }
        void vscode.env.openExternal(vscode.Uri.parse(url));
      })
      .catch(() => { /* no browser URL for this method */ });
    await authPromise;
  }

  /** The live connection for an agent (via its active session), if connected. */
  private connectionForAgent(agentName: string): ClientSideConnection | undefined {
    const sid = this.agentSessions.get(agentName);
    const sess = sid ? this.sessions.get(sid) : undefined;
    if (!sess) { return undefined; }
    return this.connectionManager.getConnection(sess.agentId)?.connection;
  }

  /** Advertised auth methods for an agent (from its initialize response). */
  authMethods(agentName: string): { id: string; name: string; description?: string }[] {
    const sid = this.agentSessions.get(agentName);
    const sess = sid ? this.sessions.get(sid) : undefined;
    const methods = (sess?.initResponse.authMethods ?? []) as { id: string; name: string; description?: string }[];
    return methods.map((m) => ({ id: m.id, name: m.name, description: m.description }));
  }

  /** Current auth status (signed-in email + advertised methods). */
  async authStatus(agentName: string): Promise<{ signedIn: boolean; email?: string; methods: { id: string; name: string; description?: string }[] }> {
    const methods = this.authMethods(agentName);
    const conn = this.connectionForAgent(agentName);
    if (!conn) { return { signedIn: false, methods }; }
    try {
      const info = (await conn.extMethod('ainxt.dev/auth/info', {})) as { email?: string };
      return { signedIn: !!info?.email, email: info?.email, methods };
    } catch {
      return { signedIn: false, methods };
    }
  }

  /** Sign in with a chosen auth method. For `ainxt.api_key`, pass `apiKey`. */
  async signIn(agentName: string, methodId: string, apiKey?: string): Promise<void> {
    let conn = this.connectionForAgent(agentName);
    if (!conn) {
      await this.connectToAgent(agentName);
      conn = this.connectionForAgent(agentName);
    }
    if (!conn) { throw new Error('AiNxt is not connected'); }
    if (apiKey !== undefined && apiKey !== '') {
      await conn.extMethod('ainxt.dev/setApiKey', { key: apiKey });
    }
    // The API-key path is complete after setApiKey (it persists the key and sets
    // the live env). Only interactive methods (OIDC/device) need the browser
    // `authenticate` handshake — running it for api_key hits agents that don't
    // implement `authenticate` and surfaces a spurious "Method not found".
    const isApiKey = /api.?key/i.test(methodId);
    if (!isApiKey) {
      await this.driveAuthBrowser(conn, methodId);
    }
  }

  /** Sign out — clears the agent's stored credentials. */
  async signOut(agentName: string): Promise<void> {
    const conn = this.connectionForAgent(agentName);
    if (conn) {
      try { await conn.extMethod('ainxt.dev/auth/logout', {}); } catch { /* ignore */ }
    }
  }
  ...
  async sendPrompt(sessionId: string, text: string): Promise<PromptResponse> {
    ...
    // Log prompt length only — never log prompt content (may contain sensitive data)
    log(`sendPrompt: session=${sessionId}, length=${text.length}`);
