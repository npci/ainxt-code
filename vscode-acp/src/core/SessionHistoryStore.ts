...
export class SessionHistoryStore {
   ...
   *
   * `lastActiveAt` is `Date.toISOString()`, so its resolution is one
   * millisecond. Two sessions stamped in the same millisecond compare equal,
   * and because `Array.prototype.sort` is stable that left them in insertion
   * order — i.e. oldest first, the exact opposite of this method's contract.
   * That misordered the session tree, and worse, `enforceCap` evicts
   * `list().slice(capPerAgent)`, so on a tie it could drop the *newest*
   * session instead of the stalest one. Ties are therefore broken explicitly:
   * `createdAt` descending, then insertion index descending (a later insertion
   * is the more recent session).
   */
  list(agentName: string, cwd?: string): PersistedSessionEntry[] {
    return this.entries
      .map((entry, index) => ({ entry, index }))
      .filter(({ entry: e }) => e.agentName === agentName && (!cwd || e.cwd === cwd))
      .sort(
        (a, b) =>
          (b.entry.lastActiveAt || '').localeCompare(a.entry.lastActiveAt || '') ||
