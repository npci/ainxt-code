...
- `AcpClientImpl` (in [agent_management](agent_management.md)) receives ACP requests from the agent process and dispatches them to the appropriate handler.
- `SessionManager` (in [session_management](session_management.md)) wires `SessionUpdateHandler` listeners so that session notifications reach the UI.
...
- `PermissionHandler` prefers the in-chat permission card via [permissionBridge](extension_ui.md) and falls back to a native VS Code QuickPick.
...
- [handlers_file_system](handlers_file_system.md) â€” reading and writing workspace text files.
- [handlers_permission](handlers_permission.md) â€” requesting and recording user permissions.
- [handlers_session_update](handlers_session_update.md) â€” routing session update notifications to UI listeners.
- [handlers_terminal](handlers_terminal.md) â€” creating, monitoring, and releasing terminals for agents.
...
| `Logger` | [utils](utils.md) | Structured logging and error reporting. |
| `checkpoints` | [agent_management](agent_management.md) | Snapshot file content before writes for revert support. |
| `permissionBridge` | [extension_ui](extension_ui.md) | In-chat permission card UI. |
| `ChatWebviewProvider` | [extension_ui](extension_ui.md) | Receives session update notifications. |
| `AcpClientImpl` | [agent_management](agent_management.md) | Dispatches ACP requests to handlers. |
| `SessionManager` | [session_management](session_management.md) | Registers session update listeners. |
