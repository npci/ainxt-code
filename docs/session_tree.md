...
The **Session Tree** module renders the **ACP Agents** sidebar view in the VS Code extension. It implements a two-tier tree: the first tier shows configured agents, and the second tier shows the sessions available for each agent. The module decides whether to list sessions from the agent itself (via the ACP `session/list` capability) or from the local [`SessionHistoryStore`](session_management.md#sessionhistorystore), and it surfaces status, errors, authentication requirements, and pagination affordances to the user.

This module is intentionally focused on **presentation and view state**. The actual lifecycle of sessions, connections, and capability discovery is delegated to the [`SessionManager`](session_management.md#sessionmanager) and related core services.
...
| Dependency | Purpose |
|------------|---------|
| `sessionManager: SessionManager` | Provides connectivity, capability summaries, and `session/list` calls. See [`SessionManager`](session_management.md#sessionmanager). |
| `historyStore: SessionHistoryStore \| null` | Local fallback for agents that support `session/load` or `session/resume` but not `session/list`. See [`SessionHistoryStore`](session_management.md#sessionhistorystore). |
...
- Clicking fires `acp.openSession` with `{ agentName, sessionId }`, which is handled by the chat webview module. See [`ChatWebviewProvider`](chat_webview.md#chatwebviewprovider).
...
| `session/load` or `session/resume` (but not `list`) | Local [`SessionHistoryStore`](session_management.md#sessionhistorystore) is the source of truth. |
...
The tree items reference the following VS Code commands. These commands are registered by the extension activation module; see [`extension.ts`](extension_activation.md).

| Command | Triggered By | Purpose |
|---------|--------------|---------|
| `acp.openChat` | Connected `AgentTreeItem` | Opens the chat webview for the agent. See [`ChatWebviewProvider`](chat_webview.md#chatwebviewprovider). |
...
- Manage agent processes or connections directly (see [`agent_management.md`](agent_management.md)).
- Persist session history (see [`session_management.md`](session_management.md)).
- Render the chat webview (see [`chat_webview.md`](chat_webview.md)).
...
- [`session_management.md`](session_management.md) — `SessionManager`, `SessionHistoryStore`, and session lifecycle.
- [`agent_management.md`](agent_management.md) — `AgentManager`, `AcpClientImpl`, and process management.
- [`chat_webview.md`](chat_webview.md) — `ChatWebviewProvider` and the chat UI that sessions open into.
- [`extension_activation.md`](extension_activation.md) — Command registration and extension lifecycle.
- [`config.md`](config.md) — `AgentConfig` and `getAgentNames`.
- [`utils.md`](utils.md) — `Logger` utilities used for diagnostics.
