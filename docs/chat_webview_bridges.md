...
For the webview implementation that consumes these bridges, see [chat_webview_provider.md](chat_webview_provider.md). For the connection layer that originates the requests, see [agent_management.md](agent_management.md) and [session_management.md](session_management.md).
...
- **Agent management / ACP client** – `AcpClientImpl.extMethod` dispatches `ainxt.dev/ask_user_question` and `ainxt.dev/exit_plan_mode` through `askBridge` and `planBridge`. See [agent_management.md](agent_management.md).
- **Permission handling** – `PermissionHandler` prefers `permissionBridge` and falls back to a native QuickPick. See [handlers.md](handlers.md).
- **Chat webview provider** – `ChatWebviewProvider` registers the UI callbacks and renders the interactive cards. See [chat_webview_provider.md](chat_webview_provider.md).
- **Session management** – Session updates and agent capabilities flow through separate channels (`SessionUpdateHandler`), but the interactive requests handled by the bridges are scoped to the active session managed by `SessionManager`. See [session_management.md](session_management.md).
