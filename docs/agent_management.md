...
This module sits between the [session_management](session_management.md) orchestrator and the concrete [handlers](handlers.md) / [chat_webview](chat_webview.md) UI bridges. It does not own session state, authentication, or long-term history â€” those concerns live in sibling modules.
...
|-----------|----------------|
| [`AgentManager`](agent_management_agent_lifecycle.md) | Spawns agent processes as child processes, tracks running instances, kills agents on shutdown, and emits lifecycle events. |
| [`AcpClientImpl`](agent_management_acp_client.md) | Implements the `@agentclientprotocol/sdk` `Client` interface. Receives agent requests/notifications and delegates to the appropriate VS Code handler or UI bridge. |
| [`checkpoints`](agent_management_checkpoints.md) | Maintains a map of file paths to their pre-turn content. Supports bounded undo of agent writes by restoring only snapshotted files. |
...
- **[agent_management_acp_client](agent_management_acp_client.md)** â€” `AcpClientImpl`: ACP client protocol implementation and request routing.
- **[agent_management_agent_lifecycle](agent_management_agent_lifecycle.md)** â€” `AgentManager`: spawning, tracking, and terminating agent child processes.
- **[agent_management_checkpoints](agent_management_checkpoints.md)** â€” `checkpoints`: per-turn file snapshots and restore.
