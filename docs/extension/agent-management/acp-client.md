...
This module is a child of [`agent_management`](README.md); it works closely with [`agent_management_agent_lifecycle`](agent-lifecycle.md) (which spawns and kills the agent process) and [`agent_management_checkpoints`](checkpoints.md) (which snapshots files before writes). It also depends on the [`handlers`](../handlers/README.md) module for concrete VS Code operations and on [`session_management`](../session-management/README.md) for the connection that carries ACP messages.
...
| **Extension methods** | Handles non-standard methods `ainxt.dev/ask_user_question` and `ainxt.dev/exit_plan_mode` via the [`chat_webview`](../chat-webview/README.md) bridges. |
...
`AcpClientImpl` is instantiated by the connection layer (see [`session_management`](../session-management/README.md)) and is passed a factory function to `ClientSideConnection`. The SDK calls the factory with the `Agent` instance once the connection is ready, and the implementation stores that agent reference via `setAgent`.
...
| ------ | --------------------- | ------- |
| `requestPermission` | [`PermissionHandler`](../handlers/README.md) | Shows a permission prompt in the chat webview or a native QuickPick. |
| `sessionUpdate` | [`SessionUpdateHandler`](../handlers/README.md) | Broadcasts a session-level notification to all registered listeners (typically the chat webview). |

#### File-system methods

| Method | Handler | Purpose |
| ------ | ------- | ------- |
| `writeTextFile` | [`FileSystemHandler`](../handlers/README.md) | Writes text to a file, snapshots the old content via [`checkpoints`](checkpoints.md), and opens the file so the user sees the change. |
| `readTextFile` | [`FileSystemHandler`](../handlers/README.md) | Reads a text file, preferring unsaved editor content over disk content, with optional line/limit slicing. |

#### Terminal methods

| Method | Handler | Purpose |
| ------ | ------- | ------- |
| `createTerminal` | [`TerminalHandler`](../handlers/README.md) | Spawns a shell process and a VS Code pseudoterminal for visibility. |
| `terminalOutput` | [`TerminalHandler`](../handlers/README.md) | Returns accumulated stdout/stderr output plus optional exit status. |
| `waitForTerminalExit` | [`TerminalHandler`](../handlers/README.md) | Awaits process exit and returns the exit code / signal. |
| `killTerminal` | [`TerminalHandler`](../handlers/README.md) | Sends `SIGTERM` to the process. |
| `releaseTerminal` | [`TerminalHandler`](../handlers/README.md) | Kills the process if still running and removes internal tracking, leaving the VS Code terminal visible. |
...
| ------ | -------------- |
| [`handlers`](../handlers/README.md) | Concrete VS Code operations for files, terminals, permissions, and session updates. |
| [`chat_webview`](../chat-webview/README.md) | `askBridge` and `planBridge` route custom interactions to the chat UI. |
| [`utils`](../utils/README.md) | `log` from `Logger.ts` is used for operational tracing. |
| [`agent_management_checkpoints`](checkpoints.md) | Indirectly used when `FileSystemHandler.writeTextFile` snapshots prior content. |
| [`session_management`](../session-management/README.md) | Creates the ACP connection and supplies the `Agent` instance to `AcpClientImpl.setAgent`. |
| [`agent_management_agent_lifecycle`](agent-lifecycle.md) | Spawns the agent process that ultimately sends the requests handled here. |
...
* [`FileSystemHandler`](../handlers/README.md) logs failures via `logError` and re-throws the underlying exception.
* [`TerminalHandler`](../handlers/README.md) throws `Terminal not found: ${id}` when an operation references an unknown terminal.
* [`PermissionHandler`](../handlers/README.md) returns an `outcome: 'cancelled'` result when the user dismisses the prompt.
...
4. **Disposal** â€” this class does not hold disposable resources itself. Cleanup is handled by the handlers (`TerminalHandler.dispose`, `SessionUpdateHandler.dispose`) and by [`agent_management_agent_lifecycle`](agent-lifecycle.md) when the agent process is killed.

---

## Related Modules

* [`agent_management`](README.md) â€” parent module that groups agent lifecycle, checkpoints, and the ACP client.
* [`agent_management_agent_lifecycle`](agent-lifecycle.md) â€” spawns, tracks, and kills agent processes.
* [`agent_management_checkpoints`](checkpoints.md) â€” snapshots file content before writes so turns can be reverted.
* [`session_management`](../session-management/README.md) â€” manages ACP connections and sessions.
* [`handlers`](../handlers/README.md) â€” concrete VS Code handlers delegated to by `AcpClientImpl`.
* [`chat_webview`](../chat-webview/README.md) â€” provides the UI bridges used by `extMethod`.
* [`utils`](../utils/README.md) â€” logging and telemetry utilities.
