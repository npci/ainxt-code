...
The `session_management_connections` module is responsible for establishing, maintaining, and disposing of Agent Client Protocol (ACP) connections between the VS Code extension host and spawned agent processes. It acts as the low-level transport layer for the broader [session_management](README.md) subsystem, bridging Node.js child process stdio streams with the typed ACP SDK.
...
The module intentionally does **not** handle session history, UI state, agent spawning, or high-level orchestration. Those concerns belong to sibling modules such as [session_management_history](history.md), [session_management_orchestration](orchestration.md), and [agent_management](../agent-management/README.md).
...
`ConnectionInfo` is the canonical handle returned after a successful connection. Callers (typically [SessionManager](orchestration.md)) use it to:
...
Removes a connection from the internal map. This does **not** terminate the underlying process; callers must coordinate process shutdown separately (see [agent_management](../agent-management/README.md)).
...
|--------|-----------------|--------------|
| [agent_management](../agent-management/README.md) | `AcpClientImpl` | ConnectionManager creates and configures the local ACP client. |
| [handlers](../handlers/README.md) | `FileSystemHandler`, `TerminalHandler`, `PermissionHandler`, `SessionUpdateHandler` | Handlers are injected into `AcpClientImpl` to satisfy agent requests. |
| [utils](../utils/README.md) | `log`, `logError`, `logTraffic` | All connection steps and traffic are logged through the shared logger. |

### Upstream Consumers

| Module | Consumer | Usage |
|--------|----------|-------|
| [session_management_orchestration](orchestration.md) | `SessionManager` | Calls `connect`, `getConnection`, and `removeConnection` during session lifecycle operations. |
...
> Note: `dispose` and `removeConnection` only affect the manager's internal registry. Process termination is handled by [AgentManager](../agent-management/README.md).
...
- [session_management](README.md) — parent module overview
- [session_management_orchestration](orchestration.md) — high-level session lifecycle and consumer of this module
- [session_management_history](history.md) — persisted session metadata
- [agent_management](../agent-management/README.md) — agent spawning and the `AcpClientImpl`
- [handlers](../handlers/README.md) — request handlers injected into the ACP client
