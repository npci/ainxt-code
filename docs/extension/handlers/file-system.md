...
`FileSystemHandler` lives in `vscode-acp/src/handlers/FileSystemHandler.ts` and is part of the broader [handlers](README.md) layer. Its responsibilities are:
...
5. **Preserve undoability** by snapshotting the previous file content through the [checkpoints](../agent-management/README.md) subsystem before overwriting.
6. **Make edits visible** by opening the modified file in the active editor.

The handler does not manage connections, sessions, or permissions. Those concerns are delegated to sibling handlers and to [agent_management](../agent-management/README.md) / [session_management](../session-management/README.md).
...
- **[utils / Logger](../utils/README.md)** — Provides `log` and `logError` for observability.
- **[agent_management / checkpoints](../agent-management/README.md)** — Provides `checkpoints.snapshot(path)` to capture pre-write file state so the user (or agent) can revert the change later.
...
`FileSystemHandler` is one of four concrete handlers in the [handlers](README.md) group:

- [handlers_permission](permission.md) — User consent before sensitive operations.
- [handlers_session_update](session-update.md) — Broadcasts session state changes.
- [handlers_terminal](terminal.md) — Terminal lifecycle and output streaming.
- **handlers_file_system** — File read/write operations (this document).

`AcpClientImpl` (see [agent_management](../agent-management/README.md)) typically dispatches incoming ACP requests to the appropriate handler. File-system requests are routed to `FileSystemHandler`.
...
No retry logic, permission prompts, or user-facing notifications are implemented inside this handler. Permission gating is handled by [handlers_permission](permission.md) before the request reaches `FileSystemHandler`.

---

## Integration with the Larger System

`FileSystemHandler` is a concrete implementation of an ACP capability handler. It is instantiated and invoked from the [agent_management](../agent-management/README.md) layer (specifically `AcpClientImpl`), which manages the agent connection and routes ACP messages. The handler relies on:

- [utils](../utils/README.md) for logging.
- [agent_management](../agent-management/README.md) for checkpoint snapshots.
- VS Code:'s workspace APIs for actual file I/O.

By keeping file I/O in this dedicated handler, the system keeps [session_management](../session-management/README.md) focused on session lifecycle, [handlers_terminal](terminal.md) focused on shell interaction, and [handlers_permission](permission.md) focused on user consent.

---

## See Also

- [handlers](README.md) — Parent module grouping all ACP capability handlers.
- [handlers_permission](permission.md) — User permission requests.
- [handlers_session_update](session-update.md) — Session update broadcasting.
- [handlers_terminal](terminal.md) — Terminal creation and output.
- [agent_management](../agent-management/README.md) — Agent process and ACP client management.
- [session_management](../session-management/README.md) — Session lifecycle and state.
- [utils](../utils/README.md) — Logging and other shared utilities.
