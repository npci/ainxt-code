...
- **[extension_activation](extension_activation.md)** — Calls `initTelemetry()` during `activate()` and registers the returned reporter in `context.subscriptions`.
- **[session_management](session_management.md)** — May emit telemetry for session lifecycle actions, auth flows, and connection outcomes.
- **[agent_management](agent_management.md)** — May emit telemetry for agent spawn/kill events and tool invocations.
- **[chat_webview](chat_webview.md)** — May emit telemetry for UI interactions such as prompt submission, mode changes, and attachment usage.

> **Note:** The telemetry module does **not** depend on `utils_logging`, but the two utilities are typically used together: `Logger` writes human-readable diagnostics to the Output channel, while `TelemetryManager` sends anonymized, aggregated events to the telemetry backend. See [utils_logging](utils_logging.md) for details.
...
- [extension_activation](extension_activation.md) — Where the telemetry reporter is initialized and disposed.
- [utils_logging](utils_logging.md) — Complementary local diagnostic logging utility.
- [session_management](session_management.md) — Consumer of telemetry for session lifecycle events.
- [agent_management](agent_management.md) — Consumer of telemetry for agent operations.
- [chat_webview](chat_webview.md) — Consumer of telemetry for UI interaction events.
