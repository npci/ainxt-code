...
|-----------|----------------|---------------|
| **Agent Management** | Spawns agent child processes, implements the ACP client protocol, and tracks per-turn file checkpoints. | [`agent_management.md`](agent_management.md) |
| **Session Management** | Orchestrates ACP connections, session creation/resume/load, active session state, auth flows, and client-side history. | [`session_management.md`](session_management.md) |
| **Handlers** | Bridges ACP tool requests to VS Code APIs: file system, terminal, permissions, and session updates. | [`handlers.md`](handlers.md) |
| **Extension UI** | Host-side UI: activation, chat webview provider, session tree, status bar, and interactive bridges. | [`extension_ui.md`](extension_ui.md) |
| **Webview UI** | React front-end bundle rendered inside the chat webview. | [`webview_ui.md`](webview_ui.md) |
| **Config** | Reads `acp.agents` / `ainxt.*` settings, injects secrets, and fetches the public ACP registry. | [`config.md`](config.md) |
| **Utils** | Logging, telemetry, and Node.js-to-Web-Streams adapters. | [`utils.md`](utils.md) |
...
- [`extension_ui.md`](extension_ui.md)
- [`webview_ui.md`](webview_ui.md)
- [`config.md`](config.md)
- [`utils.md`](utils.md)