...
- **[vscode_host](vscode_host.md)**: The IntelliJ plugin shares the same React web UI and postMessage contract as the VS Code host. Where VS Code uses a `WebviewView`, IntelliJ uses a JCEF browser, but the UI layer is identical.
- **[vscode_acp](vscode_acp.md)**: The IntelliJ `AcpClient` implements the same ACP JSON-RPC protocol as the VS Code ACP client (`AcpClientImpl`, `SessionManager`, etc.). The agent process and session semantics are the same; only the host language and IDE APIs differ.
