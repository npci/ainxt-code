...
1. **Agent spawn**: [`agent_management`](agent_management.md) creates a child process for the agent.
2. **Stream adaptation**: The raw `ChildProcess` is passed to `childProcessToWebStreams`.
3. **Connection setup**: The resulting `AcpStream` is handed to [`session_management`](session_management.md) for connection lifecycle management.
...
For details on how connections are established and torn down, see [session_management](session_management.md). For agent lifecycle management, see [agent_management](agent_management.md). For logging of traffic that flows through these streams, see [utils_logging](utils_logging.md).
...
- [agent_management](agent_management.md) — Spawns and manages agent child processes whose streams are adapted here.
- [session_management](session_management.md) — Consumes `AcpStream` instances to establish and manage ACP sessions.
- [utils_logging](utils_logging.md) — Logs traffic that flows through the adapted streams.
- [utils_telemetry](utils_telemetry.md) — Records extension-level events, including connection outcomes.
