...
This module is the execution foundation of the `agent_management` subsystem. While [agent_management_acp_client](agent_management_acp_client.md) handles the protocol-level communication with a running agent, this module is concerned with the operating-system process that hosts the agent.
...
These events allow upstream components such as [SessionManager](session_management.md) and [ChatWebviewProvider](chat_webview.md) to surface agent status to the user.
...
- **[SessionManager](session_management.md)**: Decides when to spawn or reconnect to an agent based on user session state. It calls `spawnAgent()` when a new local agent session is needed and `killAgent()` / `killAll()` during cleanup.
- **[AcpClientImpl](agent_management_acp_client.md)**: After an agent is spawned, `AcpClientImpl` communicates with it over stdin/stdout. It may retrieve the `AgentInstance` from `AgentManager` to access the process streams.
- **[extension.ts](extension_activation.md)**: Registers the extension lifecycle and calls `AgentManager.dispose()` on deactivation to ensure no orphan processes remain.
- **[Logger](utils.md)** and **[TelemetryManager](utils.md)**: Used for operational logging and telemetry events.
- **[AgentConfig](config.md)**: Supplies the `AgentConfigEntry` (command, args, environment) used to launch each agent.

## References

- [agent_management_acp_client](agent_management_acp_client.md) — protocol client that communicates with spawned agents.
- [agent_management_checkpoints](agent_management_checkpoints.md) — checkpointing support for agent sessions.
- [session_management](session_management.md) — orchestrates sessions and drives agent spawning.
- [extension_activation](extension_activation.md) — extension entry point and deactivation cleanup.
- [chat_webview](chat_webview.md) — UI surface that displays agent status and errors.
