...
> **Note:** The diagram above illustrates typical consumers. For details on how those modules operate, refer to their respective documentation pages: [agent_management](agent_management.md), [session_management](session_management.md), [handlers](handlers.md), [extension_ui](extension_ui.md), [utils_streaming](utils_streaming.md), and [utils_telemetry](utils_telemetry.md).
...
- [utils_streaming](utils_streaming.md) — Adapts child process streams to web streams for ACP communication.
- [utils_telemetry](utils_telemetry.md) — Sends telemetry events; may use logging for local diagnostics.
- [agent_management](agent_management.md) — Spawns and communicates with agents; produces traffic logs.
- [session_management](session_management.md) — Manages sessions and connections; produces traffic logs.
- [handlers](handlers.md) — Implements ACP handlers; consumes logging for file, terminal, and permission operations.
- [extension_ui](extension_ui.md) — Provides chat webview, session tree, and status bar UI; uses logging for diagnostics.
