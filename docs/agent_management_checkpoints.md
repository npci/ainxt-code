...
The diagram above highlights the checkpoint module in green. For details on how the agent process is spawned and how the ACP client routes requests, see [agent_management_agent_lifecycle](agent_management_agent_lifecycle.md) and [agent_management_acp_client](agent_management_acp_client.md).
...
- **[handlers](handlers.md)** — `FileSystemHandler.writeTextFile` calls `checkpoints.snapshot()` before persisting new content.
- **[chat_webview](chat_webview.md)** — `ChatWebviewProvider` calls `checkpoints.begin()` when the user sends a prompt, reads `checkpoints.count()` to decide whether to show a revert action, and calls `checkpoints.restore()` when the user confirms a revert.
- **[session_management](session_management.md)** — owns the prompt/cancel lifecycle that drives the turn during which checkpoints are collected.
...
- `AcpClientImpl` does not interact with checkpoints itself; it routes file requests to `FileSystemHandler`. See [agent_management_acp_client](agent_management_acp_client.md) for the full routing table.
- `ChatWebviewProvider` is the orchestrator of the turn lifecycle from the UI side. See [chat_webview](chat_webview.md) for webview message handling.
...
- **File writes** — The actual file I/O path is in [handlers](handlers.md). `FileSystemHandler.writeTextFile` is the only production caller of `checkpoints.snapshot`.
- **ACP client** — [agent_management_acp_client](agent_management_acp_client.md) routes `writeTextFile` from the agent to `FileSystemHandler`.
- **Agent lifecycle** — [agent_management_agent_lifecycle](agent_management_agent_lifecycle.md) manages the agent process, but checkpoints are independent of process lifetime.
- **Session management** — [session_management](session_management.md) owns `sendPrompt` and `cancelTurn`, which define the boundaries of a turn.
- **Chat webview** — [chat_webview](chat_webview.md) renders the revert affordance and forwards the user's `restoreCheckpoint` message.
- **Extension activation** — The module is imported at runtime by `FileSystemHandler` and `ChatWebviewProvider`; it does not require explicit activation logic. See [extension_activation](extension_activation.md) for the extension entry point.
...
- [agent_management_acp_client](agent_management_acp_client.md)
- [agent_management_agent_lifecycle](agent_management_agent_lifecycle.md)
- [handlers](handlers.md)
- [chat_webview](chat_webview.md)
- [session_management](session_management.md)
- [extension_activation](extension_activation.md)
