...
- Initialize telemetry via [`initTelemetry`](utils.md).
...
- Data stays local (gateway → Ollama); no cloud egress.
...
`activate` registers the following VS Code commands. Most commands delegate directly to [`SessionManager`](session_management.md) or [`ChatWebviewProvider`](chat_webview.md).
...
`activate` binds several [`SessionManager`](session_management.md) events to the chat webview so the UI stays in sync with the underlying session state.
...
- [agent_management](agent_management.md) — spawning and managing agent processes.
- [session_management](session_management.md) — session lifecycle, connections, and history.
- [handlers](handlers.md) — ACP request handlers for files, terminals, permissions, and updates.
- [extension_ui/chat_webview](chat_webview.md) — chat webview provider and bridges.
- [extension_ui/session_tree](session_tree.md) — sessions tree view.
- [extension_ui/status_bar](status_bar.md) — status bar updates.
- [config](config.md) — agent configuration and registry client.
- [utils](utils.md) — logging, telemetry, and stream utilities.
