...
The host can send model state in two shapes: an object with `currentModelId`/`availableModels`, or an array where the current model is flagged with `isCurrent`. `normalizeModels` collapses both into a consistent `{ current, list }` shape used by the model picker in [webview_ui_app](webview_ui_app.md).
...
- [webview_ui_app](webview_ui_app.md) imports `post`, `onHost`, `normalizeModels`, and the type interfaces to drive the chat UI.
- [chat_webview](chat_webview.md) (`ChatWebviewProvider`) is the VS Code host counterpart: it receives `UiToHost` messages and emits `HostMessage` notifications.
- [session_management](session_management.md) produces the `AcpUpdate` stream that the bridge forwards as `sessionUpdate` messages.
- [agent_management](agent_management.md) owns the tools whose progress and permission requests surface through the bridge as `permissionRequest`, `askRequest`, and `planApprovalRequest` messages.
- [webview_ui_markdown](webview_ui_markdown.md) renders assistant messages locally but does not use the bridge directly.
...
This lets [webview_ui_app](webview_ui_app.md) render the model picker without caring which host produced the payload.
...
- [webview_ui_app](webview_ui_app.md) — React application that consumes this bridge.
- [webview_ui_markdown](webview_ui_markdown.md) — markdown rendering used by the chat UI.
- [chat_webview](chat_webview.md) — VS Code host provider that implements the other side of this contract.
- [session_management](session_management.md) — produces session updates forwarded through the bridge.
- [agent_management](agent_management.md) — owns agent tools whose progress and approvals are surfaced through the bridge.
