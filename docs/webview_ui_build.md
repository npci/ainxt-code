...
- [webview_ui_app](webview_ui_app.md) — The React components (`App.tsx`, `Markdown.tsx`) that this build configuration bundles.
- [webview_ui_bridge](webview_ui_bridge.md) — The message bridge (`bridge.ts`) that is bundled alongside the UI and handles communication between the webview and the extension host.
- [chat_webview](chat_webview.md) — The extension-side provider (`ChatWebviewProvider.ts`) that loads the built assets into the webview panel using `getHtmlContent` and `asWebviewUri`.
