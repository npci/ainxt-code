...
The module is intentionally thin: all business logic (session management, agent spawning, file system operations, terminal handling) lives in sibling modules such as [`session_management`](session_management.md), [`agent_management`](agent_management.md), and [`handlers`](handlers.md). The webview UI only renders state and forwards user actions to the host.

## Sub-modules

| Sub-module | File(s) | Responsibility |
|------------|---------|----------------|
| [webview_ui_bridge](webview_ui_bridge.md) | `bridge.ts` | Defines the host â†” UI message contract and abstracts `vscode.postMessage` vs JetBrains `__ainxtHostPost`. |
| [webview_ui_app](webview_ui_app.md) | `App.tsx` | Main React application: message list, composer, activity indicators, permission/ask/plan dialogs, diff views, status bar. |
| [webview_ui_markdown](webview_ui_markdown.md) | `Markdown.tsx` | Renders agent markdown with syntax highlighting, GitHub-flavored markdown, and copy-to-clipboard code blocks. |
| [webview_ui_build](webview_ui_build.md) | `vite.config.ts` | Vite build configuration that emits a predictable single JS/CSS bundle for the extension to load. |
...
See [`extension_ui`](extension_ui.md) and [`intellij_host`](intellij_host.md) for how each host loads and drives this UI.
...
- **Extension host**: [`ChatWebviewProvider`](extension_ui.md) (VS Code) or [`AinxtToolWindowFactory`](intellij_host.md) (IntelliJ).
- **Core logic modules**: [`session_management`](session_management.md), [`agent_management`](agent_management.md), [`handlers`](handlers.md).

## Related Documentation

Detailed documentation for each sub-module:

- [webview_ui_bridge](webview_ui_bridge.md) — host ↔ UI message contract and cross-IDE bridge.
- [webview_ui_app](webview_ui_app.md) — main chat UI, composer, diff rendering, and dialogs.
- [webview_ui_markdown](webview_ui_markdown.md) — markdown rendering with syntax highlighting.
